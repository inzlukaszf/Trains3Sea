...
export default function App() {
  ...
  const [viaStations, setViaStations] = useState([])   // [{id, name, coords}]
  ...
  // Capital click on map — cycle: from → to → via
  ...
  const handleViaToggle = useCallback((city) => {
    setViaStations((prev) => {
      const exists = prev.some((v) => v.id === city.hafasId)
      if (exists) return prev.filter((v) => v.id !== city.hafasId)
      return [...prev, { id: city.hafasId, name: city.name, coords: city.coords }]
    })
  }, [])
